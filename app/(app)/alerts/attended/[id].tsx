...
export default function AttendedDetails() {
  ...
  const handleLiberate = () => {
    RNAlert.alert("Confirmar liberación", "¿Deseas liberar la alerta?", [
  ...
  useEffect(() => {
    const loadAlert = async () => {
      ...
      setError(null);
      try {
        const alertData: AlertResponse = await fetchAlert(
          id as string,
          session!
        );
  ...
  const { formattedDate: alertDate, formattedTime: alertTime } = datetimeFormat(
    alert?.alert_timestamp || ""
  );
  const { formattedTime: attendedTime } = datetimeFormat(
        ...
        >
          <View>
            <View style={styles.alertHeader}>
              ...
              />
              <Text style={[styles.h1, { color: Theme.colors.green }]}>
                ALERTA RESUELTA
              </Text>
            </View>
            <View style={styles.infoContainer}>
              ...
              <Text
                style={[
                  styles.p,
                  styles.highlight,
                  { marginTop: Theme.margin.horizontal },
                ]}
              >
                Diagnóstico:
              </Text>
              ...
              <View style={styles.bulletContainer}>
                <Text style={styles.p}>• {alert.attended_by.name}</Text>
              </View>
